## 1. Main Entry ----
#' Regulatory Guide 1.99 Rev. 2 Position 1.1 Embrittlement Property Calculator
#'
...
RG199R2_P1 <- function(
  ...
  if (!is.null(product_form)) {
    ...
    if (any(!product_form %in% c("B", "F", "P", "W"))) {
      stop("Invalid product_form")
    }
...
rg199r2_p1_tts <- function(product_form, Cu, Ni, fluence) {
  ...
  fl <- rg199r2_p1_ff(fluence)
  cf * fl
}


rg199r2_p1_ff <- function(fluence) {
  f19 <- fluence / 1e19
...
rg199r2_p1_margin <- function(product_form, Cu, Ni, fluence) {
  ...
  res <- ifelse(margin > tts, tts, margin) # The smaller of TTS or Margin
  res
}



## 3. Table-Based CF Functions ----
...
rep_expand <- function(x, max_len) {
  ...
  res <- if (length(x) == 1 && max_len > 1) rep(x, max_len) else x
  res
}
