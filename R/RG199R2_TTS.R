...
#' RG199R2_TTS(cf = 100, fluence = 3e19, output = "TTS",
#'         input_unit = "Fahrenheit", output_unit = "Celsius")
#'
#' # Compute Fluence Factor
#' RG199R2_TTS(fluence = 5e19, output = "FF")
#'
#' # Compute Regulatory Margin
#' RG199R2_TTS(cf = 80, fluence = 2e19, sd = 17,
#'         input_unit = "Fahrenheit", output = "Margin")
#'
#' @seealso \code{\link{RG199R2_P11}}, \code{\link{RG199R2_P21}}
#' @export
RG199R2_TTS <- function(cf = NULL, # for TTS, Margin
  ...
  result <- switch(
    output,
    "TTS" = rg199r2_tts(cf, fl),
    "FF" = rg199r2_ff(fl),
    "Margin" = rg199r2_margin(cf, fl, sd)
  )
...
#' RG199R2_P11(product_form = "B", Cu = 0.2, Ni = 0.8, output = "CF", output_unit = "Fahrenheit")
#'
#' # Get standard deviation for weld material
#' RG199R2_P11(product_form = "W", output = "SD")
#'
#' @seealso \code{\link{RG199R2_TTS}}, \code{\link{RG199R2_P21}}
#' @export

RG199R2_P11 <- function(product_form,
                       Cu = NULL,
                       Ni = NULL,
                       output = c("CF", "SD"),
                       output_unit = c("Celsius", "Fahrenheit"),
  ...
  if (output == "CF") {
    ...
    cu <- expanded[[2]]
    ni <- expanded[[3]]
    result <- rg199r2_p11_cf_degF(pf, cu, ni)

  } else if (output == "SD") {
    result <- rg199r2_p11_sd_degF(product_form)
  }
...
# P1.1 CF
rg199r2_p11_cf_degF <- function(product_form, Cu, Ni) {
  ...
  for (i in seq_len(n)) {
    ...
    cf[i] <- if (pf == "B") rg199r2_p11_cf_base(cu, ni) else rg199r2_p11_cf_weld(cu, ni)
  }
  cf
}
...
# P1.1 SD
rg199r2_p11_sd_degF <- function(product_form) {
  product_form <- to_baseweld(product_form)
...
# P1.1 CF base table
rg199r2_p11_cf_base <- function(Cu, Ni) { # not vectorized
  cf_base <- matrix(
    c(
...
# P1.1 CF weld table
rg199r2_p11_cf_weld <- function(Cu, Ni) { # not vectorized
  cf_weld <- matrix(
    c(
...
#'
#' @examples
#' RG199R2_P21(SV_flu = c(1e19, 2e19), SV_tts = c(30, 45), product_form = "B", output = "SD")
#'
#' @seealso \code{\link{RG199R2_TTS}}, \code{\link{RG199R2_P11}}
#'
#' @export
RG199R2_P21 <- function(SV_flu, # A numeric vector of length 2 or more
  ...
  result <- switch(output,
    "CF" = rg199r2_p21_cf(SV_flu, SV_tts_degF),
    "SD" = rg199r2_p21_sd_degF(SV_flu, SV_tts_degF, product_unique)
  )
...
# P2.1 CF
rg199r2_p21_cf <- function(SV_flu, SV_tts) {
...
# P2.1 SD
rg199r2_p21_sd_degF <- function(SV_flu, SV_tts, product_unique) {
  cf <- rg199r2_p21_cf(SV_flu, SV_tts) # Single value
