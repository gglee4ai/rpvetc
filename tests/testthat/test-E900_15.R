...
test_that("e900_tts1 computes correct TTS1 component", {
  val <- e900_tts1("P", Ni = 0.18, Mn = 1.36, P = 0.012, temperature = 290, fluence = 2.56894e18)
  expect_equal(round(val, 2), c(P = 7.85))
})

test_that("e900_tts2 computes correct TTS2 component", {
  val <- e900_tts2("P", Cu = 0.2, Ni = 0.18, P = 0.012, temperature = 290, fluence = 2.56894e18)
  expect_equal(round(val, 2), c(P = 23.90))
})

test_that("e900_tts equals tts1 + tts2", {
  tts_total <- e900_tts("P", Cu = 0.2, Ni = 0.18, Mn = 1.36, P = 0.012, temperature = 290, fluence = 2.56894e18)
  expect_equal(round(tts_total, 2), c(P = 31.74))
})

test_that("e900_sd computes SD correctly", {
  tts_val <- 31.74
  val <- e900_sd("P", tts = tts_val)
  expect_equal(round(val, 2), c(P = 11.58))
})

test_that("e900_cf1 computes normalized CF1 value", {
  val <- e900_cf1("P", Ni = 0.18, Mn = 1.36, P = 0.012, temperature = 290)
  expect_equal(round(val, 3), c(P = 17.015))  # Approximate check
})

test_that("e900_cf2 computes normalized CF2 value", {
  val <- e900_cf2("P", Cu = 0.2, Ni = 0.18, P = 0.012, temperature = 290)
  expect_equal(round(val, 4), c(P =  31.78660))  # Approximate check
})

test_that("e900_ff1 returns normalized fluence function", {
  val <- e900_ff1(2.56894e18)
  expect_equal(round(val, 4), round(0.4611635, 4))
})

test_that("e900_ff2 returns normalized fluence function", {
  val <- e900_ff2(2.56894e18)
  raw <- 113.87 * (log(2.56894e18) - log(4.5e16))
  clamped <- min(max(raw, 0), 612.6)
  expect_equal(round(val, 4), round(clamped / 612.6, 4))
})

test_that("e900_tts_by_cf computes total TTS from CF1 and CF2", {
  cf1 <- 0.2437
  cf2 <- 0.7405
  fluence <- 2.56894e18
  val <- e900_tts_by_cf(cf1, cf2, fluence)
